...
export async function setupCharacter(opts) {
  ...
  const { connId, character: detected } = await detectCharacter({
    accessToken, proxyUrl, wsUrl, timeoutMs,
  });

  const warnings = [];
  if (!detected) {
    warnings.push('no character detected from server within timeout (player may be new or rate-limited)');
  }
  ...
  if (mode === 'skip') {
    reason = detected
      ? `mode=skip — keeping server's character "${detected.name}" on ${detected.boatId}`
      : 'mode=skip — server has no character (or detection failed); not setting';
  } else if (mode === 'config') {
    if (!detected) {
      apply = true;
