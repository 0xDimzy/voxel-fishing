...
export class VoxelAPI {
   ...
   *  Returns a normalized object so callers don't have to try/catch on the
   *  common `incomplete_set` case (player just doesn't have all relics yet).
   *  - { ok: true, ...payload }            — server confirmed
   *  - { ok: false, reason: 'incomplete_set' } — no relics to claim (normal)
   *  - throws                              — network / 5xx (real errors)
   */
  async claimRelicSet(opts = {}) {
    try {
      const data = await this._request('POST', '/api/me/relic-set-claim', undefined, opts);
      return { ok: true, ...data };
    } catch (e) {
      if (e.status === 400 && e.body?.error === 'incomplete_set') {
  ...
  // Character setup moved to lib/character.js. Use `setupCharacter` directly
  // (it now does detect-first, see SKILL.md and setupMode docs in lib/character.js).
}
